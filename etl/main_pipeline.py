...
from external_data_handling import *
# from images import *
from images_no_save import *
from custom_dicom import *
from datetime import datetime
todays_date = datetime.now().strftime('%Y-%m-%d')

def subject_info(local_path,logging,program,file_dir,validate=0):
    # site_name = local_path.split('/')[1]
    logging.info('Getting subject ids.')
    ...
    if program == 'cbtn':
        ...
        # cbtn_all_fn='cbtn-all_identified_2021-11-04_updated.csv'
    ...
    if not sub_missing_c_ids.empty:
        ...
        logging.info('      WARNING: Subject(s) with missing C-IDs found. Please check  '+output_fn)
    ...
    if not sub_missing_ses.empty:
        ...
        logging.info('      WARNING: Subject(s) with missing session labels found. Please check  '+output_fn)
        sub_missing_ses.to_csv(output_fn,index=False)
    logging.info('Getting target Flywheel projects.')
#  get target Flywheel project based on diagnosis
    if program == 'cbtn':
        ...
        if not sub_missing_proj.empty:
            ...
            logging.info('      WARNING: Subject(s) not found in CBTN-all. Please check  '+output_fn)
...
def validate_info(local_path,logging,program,file_dir):
    # save a CSV of the subject mapping & DICOM fields to review
    sub_mapping,sub_missing_c_ids,sub_missing_ses,sub_missing_proj = subject_info(local_path+'DICOMs/',logging,program,file_dir,1)
...
def run_deid(local_path,s3_path,logging,program,log_path):
    file_dir = local_path+'files/'
    sub_mapping = subject_info(local_path+'DICOMs/',logging,program,file_dir)
    output_fn = file_dir+'sub_mapping_'+todays_date+'.csv'
    ...
    logging.info('Converting DICOMs to NIfTI.')
    ...
    if sub_mapping.empty:
        logging.info('NIfTI files created but no subject mapping to use.')
    else:
